...
class SendGridOfficialService:
    ...
    def send_verification_email(self, to_email: str, verification_link: str) -> bool:
        """Send email verification using official SendGrid Python library"""
        if not self.api_key:
        ...
        try:
            ...
            if response.status_code == 202:
                ...
                print(f"[ERROR] SendGrid API error: {response.status_code} - {response.body}")
                return False
    ...
    def send_password_reset_email(self, to_email: str, reset_link: str, user_name: str) -> bool:
        """Send password reset email using official SendGrid Python library"""
        if not self.api_key:
            print("[ERROR] SENDGRID_API_KEY environment variable is missing!")
            return False
...
sendgrid_official_service = SendGridOfficialService()
...
def send_verification_email(to_email: str, verification_link: str) -> bool:
    """Send verification email using official SendGrid library"""
    return sendgrid_official_service.send_verification_email(to_email, verification_link)

def send_verification_email_async(to_email: str, verification_link: str) -> None:
    """Send verification email asynchronously using official SendGrid library"""
    import threading
    
    def _send():
        success = sendgrid_official_service.send_verification_email(to_email, verification_link)
        if not success:
...
def send_password_reset_email(to_email: str, reset_link: str, user_name: str) -> bool:
    """Send password reset email using official SendGrid library"""
    return sendgrid_official_service.send_password_reset_email(to_email, reset_link, user_name)
