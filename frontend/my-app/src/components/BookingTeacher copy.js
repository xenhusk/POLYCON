...
function BookingTeacher() {
  ...
  const [SessionClicked, setSessionClicked] = useState(true);
  ...
  async function fetchStudents() {
    try {
      const response = await fetch(
        "http://localhost:5001/bookings/get_students"
      );
  ...
  async function fetchProfileDetails(userID) {
    try {
      const response = await fetch(
        `http://localhost:5001/bookings/get_user?userID=${userID}`
      );
      const data = await response.json();
      setProfileDetails({
  ...
  async function fetchTeacherAppointments() {
    ...
    try {
      const response = await fetch(
        `http://localhost:5001/bookings/get_teacher_bookings?teacherID=${teacherID}`
      );
      ...
      for (const booking of bookings) {
        ...
        const studentNames = booking.studentID.map((ref, index) => {
          ...
          return student
            ...
            : booking.studentNames && booking.studentNames[index]
            ? booking.studentNames[index]
            : "Unknown Student";
        });
  ...
  function showConfirmationInputs(bookingID, bookingItem) {
    ...
    confirmButton.onclick = () =>
      confirmBooking(bookingID, scheduleInput.value, venueInput.value);
      ...
      <div className="mb-4 relative">
        ...
        <div className="flex flex-wrap items-center gap-2 border border-gray-300 rounded-lg px-3 py-2">
          {selectedStudents.map((studentId) => {
            ...
            const studentProfile = getProfilePictureUrl(
        ...
        {appointments.pending?.map((app) => (
          ...
          >
            <div className="flex justify-between items-center">
              <div>
                <p>
                  <strong>Students:</strong> {app.studentNames}
                </p>
              </div>
              <div className="flex space-x-2">
                <button
                  onClick={() =>
                    showConfirmationInputs(
                      app.id,
                      document.getElementById(`pending-${app.id}`)
                    )
                  }
                  className="bg-blue-500 text-white px-4 py-2 rounded-lg"
                >
          ...
          >
            <div className="flex justify-between items-center">
              <div>
                ...
                <p>
                  <strong>Schedule:</strong> {formatDateTime(app.schedule)}
                </p>
                <p>
                  <strong>Venue:</strong> {app.venue}
                </p>
              </div>
              <div className="flex space-x-2">
                <button
                  onClick={() => {
                    ...
                    setTimeout(() => { setSessionClicked(false); 
                      setTimeout(() => startSession(app), 
                      500);
                    }, 200);
                  }}
                  disabled={startSession}
                  className={`bg-green-500 text-white px-4 py-2 rounded-lg
                    ${SessionClicked ? "scale-90" : "scale-100"}
                  `}
                >
                  Start Session
                </button>
                <button
                  onClick={() => {
                    ...
                    setTimeout(() => { setCancelBookingClicked(false); 
                      setTimeout(() => cancelBooking(app.id), 
                      500);
                    }, 200);
                  }}
                  className={`bg-red-500 text-white px-4 py-2 rounded-lg
                    ${CancelBookingClicked ? "scale-90" : "scale-100"}`}
                >
                  Cancel
                </button>
              </div>
          ...
          >
            <div>
              ...
              <p>
                ...
                {app.schedule && !isNaN(new Date(app.schedule).getTime())
                  ? formatDateTime(app.schedule)
                  : ""}
              </p>
              <p>
                <strong>Venue:</strong> {app.venue}
