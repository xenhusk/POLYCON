...
function AdminConsultationCalendar() {
    ...
    return (
        <div style={{ width: '100%', height: '100vh', padding: '1rem', backgroundColor: '#f8fafc' }}>
            <Calendar
                localizer={localizer}
                events={events}
                startAccessor="start"
                endAccessor="end"
                style={{ height: 'calc(100vh - 2rem)', width: '100%', backgroundColor: 'white', borderRadius: '8px', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.1)' }}
                views={['month', 'agenda']}
                defaultView="month"
                formats={{ eventTimeRangeFormat: () => '', dayFormat: 'ddd', dayHeaderFormat: 'ddd M/D' }}
                components={{ toolbar: CustomToolbar, month: { event: monthEventRenderer }, agenda: { event: eventRenderer } }}
...
export default AdminConsultationCalendar;
